...
const milestones = [
  ...
  { name: 'Legendary Coin', icon: '/abc.webp', tapBalanceRequired: 5000000, reward: 2500000 },
];
      ...
      {milestones.filter(milestone => !claimedMilestones.includes(milestone.name)).map((milestone) => {
        ...
        return (
          ...
          <div key={milestone.name} className='bg-cards rounded-[15px] p-[14px] flex flex-wrap justify-between items-center'>
            ...
            <div className=''>
              <button
                className={`w-fit relative rounded-[8px] font-semibold py-2 px-3 ${isClaimable ? `bg-${level.class} hover:bg-[#b4b4b4] ease-in duration-200` : 'bg-btn2 cursor-not-allowed'
                  }`}
